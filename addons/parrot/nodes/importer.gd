# An importer to build dialog resources based checked CSV files
@tool
class_name Importer
extends Control
...
func _on_TargetDialog_dir_selected(dir):
	var target_dir = DirAccess.new()
	target_dir.open(dir)
	
	print_debug("Opening dialog file")
	var csv_file = FileAccess.new()
	csv_file.open(_import_file, FileAccess.READ)
	...
	while ! csv_file.eof_reached():
		...
		print_debug("Read line: %s" % JSON.stringify(line))
		if (current_id != line[dialog_id_col]):
			...
			if dialog != null:
				...
				dialog.lines = lines
				ResourceSaver.save(dialog, "%s/%s.tres" % [dir, dialog.id])
			if target_dir.file_exists("%s.tres" % current_id):
				...
				target_dir.remove_at("%s.tres" % current_id)
